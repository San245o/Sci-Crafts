...
-- Buckets are private. The app creates short-lived signed URLs for published product files.
...
-- Anonymous users can request signed URLs only for files attached to published products.
drop policy if exists "Published product images can be signed" on storage.objects;
create policy "Published product images can be signed"
